...
public class UploadUtil {
    ...
    private void toUploadFile(File file, String fileKey, String RequestURL, Map<String, String> param) {
        ...
        try {
            ...
            if (res == 200) {
                LogUtil.e(TAG, "file upload request success");
                // InputStream input = conn.getInputStream();

				/*
                 * 返回信息 BufferedReader br = new BufferedReader(new
				 * InputStreamReader(conn.getInputStream(), "utf-8"));
				 * 
				 * StringBuffer sb1 = new StringBuffer(); int ss; while ((ss =
				 * br.read()) != -1) { sb1.append((char) ss); } result =
				 * sb1.toString(); Log.e(TAG, "result : " + result);
                ...
                sendMessage(UPLOAD_SERVER_ERROR_CODE, response.toJSONString());
                return;
            }
    ...
    public void uploadJson(String vrequestUrl, String vparam, OnUploadProcessListener listener) {
        final String requestUrl=vrequestUrl;
        final String param=vparam;
        this.onUploadProcessListener=listener;
        try {
    ...
    private void toUploadJson(String RequestURL, String param) {
        try {
            URL url=new URL(RequestURL);
            HttpURLConnection conn=(HttpURLConnection) url.openConnection();
            conn.setReadTimeout(readTimeOut);
